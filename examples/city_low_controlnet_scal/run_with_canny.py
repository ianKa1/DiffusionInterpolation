"""
Cyberpunk City to Modern City interpolation with Canny ControlNet.

Interpolates between cyberpunk_city.png and modern_city.png using Canny edge
...
img1 = Image.open(os.path.join(script_dir, 'cyberpunk_city.png')).convert('RGB').resize((512, 512))
img2 = Image.open(os.path.join(script_dir, 'modern_city.png')).convert('RGB').resize((512, 512))

print(f"Image 1 (cyberpunk city): {img1.size}")
print(f"Image 2 (modern city): {img2.size}")

prompt = (
    'cyberpunk city, neon lights, futuristic, night, high quality, detailed, sharp',
    'modern city, daytime, clean architecture, high quality, detailed, sharp'
)
n_prompt = (
...
qc_prompt = 'city, urban, detailed, high quality, clear, sharp'
...
results = interpolator.interpolate_qc(
    ...
    optimize_cond=0,
    use_controlnet=True,
    controlnet_conditioning_scale=0.25,   # uniform low scale for dense city edges
    latent_interp='slerp',
...
gif_path = os.path.join(output_dir, 'cyberpunk_to_modern.gif')
results[0].save(
    gif_path,
    save_all=True,
...
print(f"\n{'='*70}")
print("Cyberpunk → Modern City Interpolation Complete!")
print(f"{'='*70}")
